...
void test_tag_encryption_decryption() {
    ...
    assert(are_bignums_equal(input_blinding_cipher, tag_decrypted));
}


void test_verify_one_bit_error_in_ciphertext() {
    ...
    assert(ciphertext_blocks[0].chunks[2] != correct_ciphertext_block.chunks[2]);
    ...
    assert(res.ciphertext_blocks[0].chunks[2] == correct_ciphertext_block.chunks[2]);
}


void test_verify_two_bit_error_in_ciphertext() {
    char *plaintext = "Hallo Welt! Ich bin der Jonas.";
    int threshold = 2;
    ...
    assert(ciphertext_blocks[0].chunks[2] != correct_ciphertext_block.chunks[2]);
    assert(ciphertext_blocks[0].chunks[0] != correct_ciphertext_block.chunks[0]);
    ...
    assert(res.correction_successful);
    assert(res.ciphertext_blocks[0].chunks[2] == correct_ciphertext_block.chunks[2]);
    assert(res.ciphertext_blocks[0].chunks[0] == correct_ciphertext_block.chunks[0]);
}


void test_verify_one_bit_error_in_tag() {
    char *plaintext = "Hallo Welt! Ich bin der Jonas.";
    int threshold = 2;
    ...
    assert(tag.chunks[2] != correct_tag.chunks[2]);

    verify_result res = verify(authorized_data, ciphertext_blocks, nob, tag, threshold, hash_key, blinding_key, blinding_nonce);

    assert(res.error);
    assert(res.correction_successful);
    assert(res.tag.chunks[2] == correct_tag.chunks[2]);
}


void test_verify_error_in_two_blocks() {
    char *plaintext = "Hallo Welt! Ich bin der Jonas.";
    int threshold = 2;
    ...
    assert(ciphertext_blocks[0].chunks[2] != correct_ciphertext_block_0.chunks[2]);
    assert(ciphertext_blocks[1].chunks[0] != correct_ciphertext_block_1.chunks[0]);
    ...
    assert(!res.correction_successful);
    for (int i = 0; i < nob; i++) {
        assert(are_bignums_equal(ciphertext_blocks[i], res.ciphertext_blocks[i]));
    }
    assert(are_bignums_equal(tag, res.tag));
}


void test_verify_no_error() {
    char *plaintext = "Hallo Welt! Ich bin der Jonas.";
    int threshold = 2;
...
void test_verify_two_bit_error_and_threshold_is_one() {
    char *plaintext = "Hallo Welt! Ich bin der Jonas.";
    int threshold = 1;
...
void run_tests_magic_mode() {
    test_tag_encryption_decryption();
    test_verify_one_bit_error_in_ciphertext();
    test_verify_two_bit_error_in_ciphertext();
    test_verify_one_bit_error_in_tag();
    test_verify_error_in_two_blocks();
    test_verify_no_error();
    test_verify_two_bit_error_and_threshold_is_one();
}